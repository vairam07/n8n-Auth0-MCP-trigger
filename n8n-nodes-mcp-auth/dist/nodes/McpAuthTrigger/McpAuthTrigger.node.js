...
class McpAuthTrigger {
    constructor() {
        this.description = {
            ...
            group: ['trigger'],
            version: 1,
            description: 'MCP Server Trigger that preserves and forwards the Authorization header downstream so you can validate JWT/Bearer tokens (e.g. Auth0) inside your workflow.',
            defaults: {
            ...
            properties: [
                // ── Path ──────────────────────────────────────────────────────────
                {
                    ...
                    default: 'mcp-auth',
                    required: true,
                    description: 'The URL path this MCP endpoint listens on (e.g. "eod_prices" → /webhook/eod_prices)',
                },
                ...
                // ── Reject on invalid ─────────────────────────────────────────────
                {
                    ...
                    description: 'Whether to return 401 immediately if validation fails, or pass failure info downstream',
                },
                // ── Response Mode ─────────────────────────────────────────────────
                {
    ...
    async webhook() {
        ...
        if (tokenValidation === 'static') {
            const expected = this.getNodeParameter('staticToken', '');
            if (!token) {
                tokenValid = false;
            ...
            const domain = this.getNodeParameter('auth0Domain', '');
            if (!token) {
                tokenValid = false;
                tokenError = 'No Authorization header provided';
            }
